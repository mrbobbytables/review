...
function requireReleaseAsset(release, name) {
	...
	if (!asset) throw new Error(`GH ${release.tag_name} has no ${name} asset`);
	const digest = DIGEST_PATTERN.exec(asset.digest ?? "");
	if (!digest) throw new Error(`GH ${release.tag_name} ${name} has no valid SHA-256 digest`);
	return digest[1];
}

export function releasePins(release, requestedVersion) {
	if (!release || release.draft === true || release.prerelease === true) {
		throw new Error("GH release must be a published stable release");
	}
	const version = String(release.tag_name ?? "").replace(/^v/, "");
	if (!VERSION_PATTERN.test(version)) throw new Error(`invalid GH release tag: ${release.tag_name ?? "missing"}`);
	if (requestedVersion && version !== requestedVersion) {
		throw new Error(`requested GH ${requestedVersion}, received ${version}`);
	}
	return {
		version,
		x86_64: requireReleaseAsset(release, `gh_${version}_linux_amd64.tar.gz`),
		aarch64: requireReleaseAsset(release, `gh_${version}_linux_arm64.tar.gz`),
	};
}
...
function readPinnedVersion(source, path) {
	const matches = [...source.matchAll(/^ARG GH_VERSION=(.*)$/gm)];
	if (matches.length !== 1) throw new Error(`${path}: expected one ARG GH_VERSION pin`);
	const version = matches[0][1];
	if (!VERSION_PATTERN.test(version)) throw new Error(`${path}: invalid GH version ${version}`);
	return version;
}

export function updateContainerfile(source, pins, path = "Containerfile") {
	let updated = replaceSingle(source, /^ARG GH_VERSION=.*$/gm, `ARG GH_VERSION=${pins.version}`, path);
	updated = replaceSingle(updated, /^ARG GH_X86_64_SHA256=.*$/gm, `ARG GH_X86_64_SHA256=${pins.x86_64}`, path);
	return replaceSingle(updated, /^ARG GH_AARCH64_SHA256=.*$/gm, `ARG GH_AARCH64_SHA256=${pins.aarch64}`, path);
}
...
async function fetchRelease(requestedVersion, fetchImpl) {
	...
	const headers = {
		...
		"User-Agent": "hive-contribute-gh-sync",
...
export async function syncGhPins({ root = process.cwd(), requestedVersion, fetchImpl = fetch } = {}) {
	...
	if (!normalized || !VERSION_PATTERN.test(normalized)) throw new Error(`invalid requested GH version: ${requestedVersion}`);
	if (!requestedVersion && pinnedVersions.size !== 1) {
		throw new Error(`GH versions differ across shipped images: ${[...pinnedVersions].join(", ")}`);
	}

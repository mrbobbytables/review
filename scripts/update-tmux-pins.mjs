...
const REPOSITORY = "tmux/tmux-builds";
...
function requireReleaseAsset(release, name) {
	...
	if (!asset) throw new Error(`tmux ${release.tag_name} has no ${name} asset`);
	const digest = DIGEST_PATTERN.exec(asset.digest ?? "");
	if (!digest) throw new Error(`tmux ${release.tag_name} ${name} has no valid SHA-256 digest`);
	return digest[1];
}

export function releasePins(release, requestedVersion) {
	if (!release || release.draft === true || release.prerelease === true) {
		throw new Error("tmux release must be a published stable release");
	}
	const version = String(release.tag_name ?? "").replace(/^v/, "");
	if (!VERSION_PATTERN.test(version)) throw new Error(`invalid tmux release tag: ${release.tag_name ?? "missing"}`);
	if (requestedVersion && version !== requestedVersion) {
		throw new Error(`requested tmux ${requestedVersion}, received ${version}`);
	}
	return {
		version,
		x86_64: requireReleaseAsset(release, `tmux-${version}-linux-x86_64.tar.gz`),
		aarch64: requireReleaseAsset(release, `tmux-${version}-linux-arm64.tar.gz`),
	};
}
...
function readPinnedVersion(source, path) {
	const matches = [...source.matchAll(/^ARG TMUX_VERSION=(.*)$/gm)];
	if (matches.length !== 1) throw new Error(`${path}: expected one ARG TMUX_VERSION pin`);
	const version = matches[0][1];
	if (!VERSION_PATTERN.test(version)) throw new Error(`${path}: invalid tmux version ${version}`);
	return version;
}

export function updateContainerfile(source, pins, path = "Containerfile") {
	let updated = replaceSingle(source, /^ARG TMUX_VERSION=.*$/gm, `ARG TMUX_VERSION=${pins.version}`, path);
	updated = replaceSingle(updated, /^ARG TMUX_X86_64_SHA256=.*$/gm, `ARG TMUX_X86_64_SHA256=${pins.x86_64}`, path);
	return replaceSingle(updated, /^ARG TMUX_AARCH64_SHA256=.*$/gm, `ARG TMUX_AARCH64_SHA256=${pins.aarch64}`, path);
}
...
export async function syncTmuxPins({ root = process.cwd(), requestedVersion, fetchImpl = fetch } = {}) {
	...
	if (!normalized || !VERSION_PATTERN.test(normalized)) throw new Error(`invalid requested tmux version: ${requestedVersion}`);

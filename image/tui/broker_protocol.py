...
Both host-side brokers (review-exec-broker, review-lab-broker) and container-side
TUI clients (review_exec_client, lab_client) speak the same protocol over a
unix socket: one newline-terminated JSON request carrying a version, an action,
and the session id, answered by one newline-terminated JSON response. That layer —
framing, byte caps, request and response validation, the session check, client
exchange, and the socket server lifecycle — lives here exactly once; a broker
contributes only its action set, its handlers, and its timeouts.
...
The client-side response frame loop similarly enforces the byte cap and
validates the protocol version before parsing.
"""
...
def read_response_line(connection, max_bytes: int = MAX_RESPONSE_BYTES) -> bytes:
    """Read one newline-terminated response line bounded by max_bytes.

    Reads chunks until a newline is found or EOF is reached.
    If the response exceeds max_bytes, raises ValueError.
    If EOF is reached before receiving any data, raises EOFError.
    """
    chunks: list[bytes] = []
    size = 0
    while True:
        ...
        size += len(chunk)
        if size > max_bytes:
            raise ValueError(f"broker answer exceeded the response bound ({max_bytes} bytes)")
        if b"\n" in chunk:
            break
    data = b"".join(chunks)
    if not data:
        raise EOFError("broker connection closed before response")
    return data.split(b"\n", 1)[0]
...
def client_request(
    ...
    *,
    ...
    """Send one request and read one response line over a Unix domain socket.

    Frames the request as newline-terminated JSON carrying version and session,
    sends it to path, reads the response bounded by max_response_bytes, and
    validates the response envelope (JSON object and PROTOCOL_VERSION match).
    """
    if not path:
        raise ValueError("socket path is empty")
    body = json_line({"version": PROTOCOL_VERSION, "session": session, **dict(payload)})
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as client:
        client.settimeout(timeout)
        client.connect(path)
        client.sendall(body)
        raw = read_response_line(client, max_bytes=max_response_bytes)
    try:
        answer = json.loads(raw.decode("utf-8"))
    except (UnicodeDecodeError, ValueError) as error:
        raise ValueError(f"unparseable broker answer: {error}") from error
    if not isinstance(answer, dict):
        raise ValueError("broker answer was not an object")
    if answer.get("version") != PROTOCOL_VERSION:
        raise ValueError(f"unsupported broker protocol {answer.get('version')!r}")
    return answer
